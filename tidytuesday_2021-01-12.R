...
# Soru 1: En çok esere sahip 20 sanatçınını eser sayılarının grafiğini çizdirelim.
...
# geom_col() grafikleri oluşturmak 2----
...
# Soru 1: artist verisetinde soyisim - isim ayıralım.

seperated <- artists %>% 
  separate(name, c("surname", "name"), sep = ", ", 
           fill = "right", extra = "merge") 

# Soru 2: artist verisetinde place of birthü ayraılım.

artists <- artists %>% clean_names()

birth_sep_artists <- artists %>% 
  separate(place_of_birth, c("city", "country"), sep = ", ",
           fill = "right", extra = "merge")

# Soru 3: artistlerin en fazla hangi ülkede doğduklarını grafikleyelim.
# Not: gender ile dolduralım grafikleri

birth_sep_artists %>% 
  filter(!is.na(country)) %>% 
  count(gender, country, sort = TRUE) %>% 
  head(10) %>% 
  mutate(country = fct_reorder(country, n)) %>% 
  ggplot(aes(n, country, fill = gender)) +
  geom_col() +
  labs(x = "Artist Sayısı",
       y = "Artistin Doğduğu Ülke",
       title = "Hangi ülke Doğumlu Ne Kadar Artist Var?",
       fill = "Cinsiyet") 
